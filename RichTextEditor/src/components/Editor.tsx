...
import { useQuill, useRegex } from './hooks';
import 'quill/dist/quill.snow.css';
import '../styles/fluidAnimation.css';
import { ImprovedSuggestions } from '../components';

const Editor = () => {
    const { quillRef, quillInstance, editorState } = useQuill();
        ...
        <div className="w-full max-w-4xl mx-auto p-6 relative overflow-hidden">
            ...
            <div className="glassmorphism-container relative z-10 p-4 rounded-xl shadow-xl">
                ...
                <div
                    ...
                    className={`editor-container border border-gray-200/30 rounded-lg overflow-hidden cursor-text relative z-10 transition-all duration-500 ${isEditorFocused ? 'editor-focused glassmorphism-active' : 'glassmorphism-inactive'
                        }`}
                ...
                <div className="mt-3 text-xs text-gray-600 flex justify-between relative z-10">
                    ...
                    <span>
                        {editorState && editorState.trim() !== '' ?
                            `${editorState.split(' ').filter(Boolean).length} words` :
                            'No content'
                        }
                    </span>
                </div>
